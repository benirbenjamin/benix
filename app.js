...
async function processAdminCommissions(orderId) {
  try {
    ...
    for (const item of orderItems) {
      ...
      const adminCommission = parseFloat((parseFloat(item.price) * parseFloat(adminCommissionRate) / 100 * item.quantity).toFixed(2));
    ...
    for (const item of orderItems) {
      if (item.ref_code) {
        ...
        if (sharedProducts.length > 0) {
          ...
          const userCommission = parseFloat((adminCommission * parseFloat(userCommissionPercentage) / 100).toFixed(2));
...
// const commission = parseFloat((costPerClick * (parseFloat(commissionRate) / 100)).toFixed(2));
...
app.listen(PORT, async () => {
  try {
    ...
    // Initialize database with automatic table creation/updates
